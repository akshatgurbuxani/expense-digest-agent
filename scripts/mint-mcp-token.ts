import { makeMcpTokenService, makeSystemClock } from "@expense/config";
...
function parseArgs(): { userId: string; ttlSeconds?: number } {
  ...
  if (!userId) {
    console.error("Usage: tsx scripts/mint-mcp-token.ts --user-id=<userId> [--ttl=<seconds>]");
    console.error("");
    console.error("  --user-id   The user ID to mint a token for (required)");
    console.error("  --ttl       Token lifetime in seconds (default: 900 / 15 min)");
    process.exit(1);
  }
...
async function main(): Promise<void> {
  ...
  if (!secret || secret.length < 16) {
    ...
    console.error("  dotenv -e .env -- tsx scripts/mint-mcp-token.ts --user-id=xxx");
    process.exit(1);
  }
